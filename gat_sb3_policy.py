...
class _UnusedDictExtractor(BaseFeaturesExtractor):
    """
    Minimal extractor required by ActorCriticPolicy.

    The actual graph processing is performed by
    GraphAttentionSchedulerNetwork.
    """
...
class DirectedGATBlock(nn.Module):
    """
    Bidirectional graph-attention block.

    One GAT receives predecessor -> successor edges.
    Another GAT receives reversed edges.
    """
    ...
    def __init__(
        ...
        head_dimension = hidden_dim // heads
    ...
    def forward(
        ...
        messages = F.elu(
            self.fusion(messages)
        )

        messages = self.dropout(
            messages
        )
...
class GraphAttentionSchedulerNetwork(nn.Module):
    ...
    def __init__(
        ...
        if gat_layers <= 0:
            raise ValueError(
                "gat_layers must be at least 1."
            )

        if gat_hidden_dim % gat_heads != 0:
            raise ValueError(
                "gat_hidden_dim must be divisible "
                "by gat_heads."
            )

        node_shape = observation_space[
            "node_features"
        ].shape

        core_shape = observation_space[
            "core_features"
        ].shape

        global_shape = observation_space[
            "global_features"
        ].shape
        ...
        if int(action_space.n) != expected_actions:
            raise ValueError(
                "Action space size must equal "
                "max_tasks * num_cores: "
                f"expected {expected_actions}, "
                f"received {action_space.n}."
            )
    ...
    def _initialize_dense_layers(
        ...
        final_action_layer = (
            self.action_scorer[-1]
        )

        nn.init.orthogonal_(
            final_action_layer.weight,
            gain=0.01,
        )
        nn.init.zeros_(
            final_action_layer.bias
        )
        ...
        nn.init.orthogonal_(
            final_value_layer.weight,
            gain=1.0,
        )
        nn.init.zeros_(
            final_value_layer.bias
        )

    @staticmethod
    def _ensure_batch_dimension(
        tensor: torch.Tensor,
        expected_dimensions_without_batch: int,
    ) -> torch.Tensor:
        if (
            tensor.ndim
            == expected_dimensions_without_batch
        ):
    ...
    def _build_batched_edge_index(
        ...
        safe_source = source.clamp(
            0,
            self.max_tasks - 1,
        )

        safe_target = target.clamp(
            0,
            self.max_tasks - 1,
        )

        source_is_real = torch.gather(
            node_mask,
            1,
            safe_source,
        )

        target_is_real = torch.gather(
            node_mask,
            1,
            safe_target,
        )
    ...
    def _encode_graph(
        ...
        flat_node_mask = node_mask.reshape(
            batch_size * self.max_tasks
        )

        node_embeddings = self.node_input(
            node_features
        )
        ...
        batched_edge_index = (
            self._build_batched_edge_index(
    ...
    def forward(
        ...
        core_embeddings = self.core_encoder(
            core_features
        )
        ...
        core_summary = core_embeddings.mean(
            dim=1
        )
        ...
        node_for_pairs = (
            node_embeddings.unsqueeze(2).expand(
                batch_size,
                self.max_tasks,
                self.num_cores,
        ...
        core_for_pairs = (
            core_embeddings.unsqueeze(1).expand(
                batch_size,
                self.max_tasks,
                self.num_cores,
...
class MaskedGATActorCriticPolicy(
    ActorCriticPolicy
):
    """
    PPO policy with:
        ...
        - alpha-based exploration

    The exploration distribution is:

        mixed_probability =
            (1 - alpha) * policy_probability
            + alpha * uniform_valid_probability

    This keeps PPO log-probabilities consistent.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.core_hidden_dim = int(
            core_hidden_dim
        )
        self.global_hidden_dim = int(
            global_hidden_dim
        )
        self.context_hidden_dim = int(
            context_hidden_dim
        )
        ...
        self.set_exploration_alpha(
            exploration_alpha
        )

        kwargs[
            "features_extractor_class"
        ] = _UnusedDictExtractor
    ...
    def _distribution_and_value(
        ...
        safe_mask = self._safe_action_mask(
            action_logits,
            observations,
        )

        masked_logits = (
            action_logits.masked_fill(
                ~safe_mask,
                -1e9,
            )
        )
